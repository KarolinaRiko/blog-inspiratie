...
const uploadFitnessToFirestore = async () => {
  try {
    for (const category in fitnessDetail) {
      const exercises = fitnessDetail[category];
      for (const exercise of exercises) {
        await setDoc(doc(db, "fitnessDetail", exercise.slug), {
          ...exercise,
          category: category
        });
        console.log(`Exercițiu adăugat: ${exercise.title}`);
      }
    }
  } catch (error) {
